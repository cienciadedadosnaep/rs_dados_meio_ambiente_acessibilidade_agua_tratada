...
dados <- ssa_painel_saneamento_brasil %>% 
  select(Indicador,`População total que mora em domicílios com acesso à água tratada (pessoas) (SNIS)`,
         `População total que mora em domicílios sem acesso à água tratada (pessoas) (SNIS)`,
         `Volume de água consumida (mil m³) (SNIS)`, 
...
dados <- subset(dados, classe == "q3_vol_agua_cons")
